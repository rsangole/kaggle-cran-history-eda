...
1. What license is most used? Has there been a change over time? _done_
1. How many packages use all CAPS, all small, or a mixture?
1. How have the dependencies & imports changed over time? _done_
1. Which repositories do packages use? Github/Bitbucket etc. How do these vary over time? _done_
1. Do packages have URLs for bug reports? _done_
1. Is there any temporal patterns to when versions are submitted to CRAN?
1. Have titles & descriptions gotten longer over time? _done_
...
Another interesting way to look at the same data is by ridge plots using [`{ggridges}`](https://wilkelab.org/ggridges/). It's easy to see the large spread of descriptions and how it's been increasing over time.
...
GPL-2, GPL-3 and MIT licenses seem to have the same adoption rate over time. Again, we see the sudden shifts in datapoints available after mid-2015.
...
## Bug Report URLs {.tabset}

_Q: Do packages have URLs for bug reports?_

```{r message=FALSE, warning=FALSE}
ov_dt %>%
  group_by(dt) %>% 
  count(url_exist = is.na(url_domain)) %>%  
...
ov_dt %>% 
  group_by(dt) %>% 
  count(url_exist = is.na(bug_domain)) %>%  
  ggplot(aes(x= dt, y = n, color = url_exist)) +
  geom_jitter(alpha = 0.3) +
  geom_smooth(span = 0.3, se = FALSE) +
  theme_light()
...
_Q: Which repositories do packages use? Github/Bitbucket etc. How do these vary over time?_
...
## Temporal Patterns {.tabset}

_Q: Is there any temporal patterns to when versions are submitted to CRAN?_
...
Thanks for reading the EDA, lots more to explore in this dataset!